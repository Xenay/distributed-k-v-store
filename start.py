...
"""Cassandra node without raft, includes caching, redirection and different consistency goodies
    Redis doesn't work
    MemCache is slow per the nature of the k,v actions"""
    ...
    def __init__(self, ip, port, all_nodes, cassandra_hosts):
        ...
        try:
            self.session.execute(
                "CREATE KEYSPACE IF NOT EXISTS kvstore WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': '2'}"
            )
            self.session.execute(
                "CREATE TABLE IF NOT EXISTS kvstore.data (key text PRIMARY KEY, value text)"
            )
        except Exception as e:
            ...
            with self.active_requests_lock:
                ...
                try:
                    ...
                    result = self.session.execute(
                        f"SELECT value FROM kvstore.data WHERE key = '{key}';"
                    ).one()
                    if result:
                        value = result.value
                    ...
                    try:
                        ...
                        return JSONResponse(
                            status_code=500,
                            content={
                                "error": "Both Cassandra instances are down or unreachable"
                            },
                        )
        ...
        async def post(key: str, value: str):
            ...
            select_query = SimpleStatement(
                "SELECT value FROM kvstore.data WHERE key = %s"
            )
            ...
            insert_query = SimpleStatement(
                "INSERT INTO kvstore.data (key, value) VALUES (%s, %s)"
            )
    ...
    async def redirect_request(self, key):
        ...
        if alternative_node:
            response = await self.http_session.get(
                f"http://{alternative_node['ip']}:{alternative_node['port']}/get/{key}"
            )
    ...
    async def redirect_request(self, key):
        ...
        if node:
            response = self.http_session.get(
                f"http://{node['ip']}:{node['port']}/get/{key}"
            )
            return response.json()
        else:
...
nodes = [
    Node(node_info["ip"], node_info["port"], nodes_info, cassandra_hosts)
