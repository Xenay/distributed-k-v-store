...
class Node:
    def __init__(self, ip, port, all_nodes, cassandra_hosts, app):
        ...
        self.state = [state for state in all_nodes if state["port"] == self.port][0][
            "state"
        ]
        ...
        async def get(key: str):
            ...
            #   f"http://{redirect_node['ip']}:{redirect_node['port']}/get/{key}"
            # )
        ...
        async def post(key: str, value: str):
            ...
            print(
                f"Time needed to write this message to file was {time.time() - start_writing}"
            )
            """ DEBUG """
        ...
        @self.app.get("/load")
        async def returnLoad():
            return {"load": self.active_requests}
        ...
        async def append_entries(request: AppendEntriesRequest):
            ...
            if (request.leader_commit > (self.commit_index + 1)) and (
                len(self.log) > 0
            ):
                ...
                return {
                    "success": False,
                    "term": self.term,
                    "error": "Log index mismatch",
                    "mismatch_index": request.prev_log_index,
                }
            ...
            for node in self.all_nodes:
                if node["port"] != self.port:
                    self.http_session.get(
                        f"http://{node['ip']}:{node['port']}/turnToFollower"
                    )

    def check_if_leader_alive(self):
        print((time.time() - self.last_heartbeat_time))
        if (time.time() - self.last_heartbeat_time) > self.election_timeout and (
            self.state != "leader"
        ):
            print("timeout")
            # self.start_election()
    ...
    def append_new_entry_and_replicate(self, command):
        ...
        new_log_entry = LogEntry(
            index=len(self.log) + 1, term=self.term, command=command
        )
    ...
    def find_least_loaded_follower(self):
        ...
        for node in self.all_nodes:
            if node["port"] != self.port and node["state"] == "follower":
                ...
                print(
                    f"sending request to port: {node['port']} ------------------------------------------------------------------------------------------------"
                )
